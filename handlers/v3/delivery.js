...
import { get_full_courier } from "../../libs/couriers/store.js";
import { runCreate } from "../../libs/couriers/engine.js";
import {
  handle_payment_ref,
...
const create_delivery = async (req, opts) => {
  ...
  try {
    ...
    // The stored estimate carries courier_id (see fetch_estimates) — fetch
    // that exact courier from the db rather than matching courierName
    // against a hardcoded array. Only estimates created after couriers
    // moved to the db have courier_id; any older estimate hits the check
    // below and fails cleanly instead of dispatching to the wrong config —
    // it self-clears within the hour since estimates expire that fast
    // anyway.
    const courier_id = estimate?.courier_id;
    if (!courier_id) {
      debug("[create_delivery] Estimate has no courier_id", {
        estimate_id: details.estimate_id,
      });
      return {
        ok: false,
        message: "This estimate has expired — please request a new one",
      };
    }

    const courier = await get_full_courier(db, courier_id);
    if (!courier) {
      debug("[create_delivery] Courier not found in db", { courier_id });
      return { ok: false, message: "Courier not found" };
    }

    let reply = await runCreate(courier, details);
